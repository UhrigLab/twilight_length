...
#            self.names = 'genotypes.txt'
    ...
    img1 = pcv.white_balance(img,roi=(800, 350, 20, 20))             
    ...
    rotate_img = pcv.rotate(img1, 180, True)
    ...
    img_binary = pcv.threshold.binary(a, 125, 255, 'dark')
    #                                     ^
    #                                     |
    #                                 adjust this value
    ...
    dilated = pcv.dilate(fill_image, 3, 1)
    ...
    for j in range(8):
        for i in range(4):
            if i<4:
                coord.append((950+i*230,120+j*223))
            else:
...
# 0 6 12 18 24 30
# 3, 9, 15, 21, 27, 33...
    roi_order = np.array([0,4,8,12,16,20,24,28, 2,6,10,14,18,22,26,30, 1,5,9,13,17,21,25,29, 3,7,11,15,19,23,27,31])
